...
class lmbcs {
 public:
  ...
  static auto translate(const T& text) -> return_t<T> {
    ...
    return_t<T> out(text.size() * 2, '\0');
    const auto* inp_ptr = reinterpret_cast<const char*>(text.data());
    auto* out_ptr = reinterpret_cast<char*>(out.data());

    const auto out_len =
      translate_impl({inp_ptr, text.size()}, {out_ptr, out.size()}, std::same_as<T, lmbcs::view>);
    out.resize(out_len);
    return out;
  }
