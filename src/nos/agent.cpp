...
auto agent::run(const std::optional<dmn::note>& note) const -> std::optional<std::string> {
  ...
  const dmn::uhandle<HAGENTCTX> ctx(agent_ctx, AgentDestroyRunContext);
  ...
  result = AgentRun(agent_hdl, agent_ctx, dmn::dhandle_t{}, 0);
  result.throw_if_error("Failed to run agent");

  // Grab the output
  DWORD out_len = 0;
  dmn::dhandle_t out_hdl = {};
  ...
  if (out_hdl == dmn::dhandle_t{} || out_len > MAX_TEXT_BUFFER) {
    return std::nullopt;
  }

  try {
    auto out_obj = dmn::os::locker(out_hdl, out_len, os::ownership::borrow);
