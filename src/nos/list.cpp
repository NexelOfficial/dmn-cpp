...
static_assert(sizeof(dmn::dhandle_t) == sizeof(DHANDLE));
...
auto list::size() const -> size_t {
  auto list = dmn::os::locker(hdl_.get(), size_, dmn::os::ownership::borrow);
...
auto list::at(size_t index) const -> std::string {
  ...
  const dmn::status result =
    ListGetText(list.get_pointer(), TRUE, index, &text, &text_size);
